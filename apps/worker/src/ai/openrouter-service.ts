...
import { OpenRouterMessage, OpenRouterTemplatedMessage } from "../models/open-router-message";
...
export class OpenRouterService {
    ...
    async generateTemplatedContent<T>(message: OpenRouterTemplatedMessage): Promise<ModelResults<T>> {
        // Prepare messages
        const body = {
            messages: typeof message.prompt === 'string' ? JSON.parse(message.prompt) : message.prompt,
            ...(message.config ?? {}),
            model: message.model,
        ...
        if (message.promptName === 'compare-results') {
            console.log(body);
            console.log(typeof message.prompt);
        }
        const response = await fetch(`${this.baseUrl}/chat/completions`, {
            method: 'POST',
            headers: {
